...
class PublicApiController extends Controller
{
    ...
    public function earlyAccessStore(Request $request)
    {
        // Validate request
        $validated = $request->validate([
            ...
            'interests' => ['nullable', 'array'], // JSON array
        ]);

        // Save record
        $earlyAccess = EarlyAccess::create([
            ...$validated,
            'signup_ip' => $request->ip(), // auto capture IP
        ]);
    ...
    public function getFilteredBlogs(Request $request)
    {
        ...
        if ($tag = $request->query('tag')) {
            $query->whereJsonContains('group_tags', $tag);
        }

        // Featured / Active filters
        if (!is_null($isFeatured = $request->query('featured'))) {
            $query->where('is_featured', filter_var($isFeatured, FILTER_VALIDATE_BOOLEAN));
        }

        if (!is_null($isActive = $request->query('active'))) {
